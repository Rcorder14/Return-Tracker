...
const firebaseConfig = {
  apiKey: "AIzaSyCQrMQ0N4Y-HCgKXYraoZduSom3VGxNiDY",
  authDomain: "return-tracking-fd895.firebaseapp.com",
  projectId: "return-tracking-fd895",
  storageBucket: "return-tracking-fd895.firebasestorage.app",
  messagingSenderId: "840614551059",
  appId: "1:840614551059:web:6cdd0ac4d0156a3cf0e9e6",
  measurementId: "G-KHZ5WE7HP6"
};
...
async function addReturn() {
    ...
    if (!customerName || !customerAddress || !phone1 || !phone2 || !whatsapp || !returnDetails || !itemDetails) {
        return showAlert("Please fill all fields before submitting.", "error");
    }
    if (!/^\d{10}$/.test(phone1)) return showAlert("Phone Number 1 must be exactly 10 digits.", "error");
    if (!/^\d{10}$/.test(phone2)) return showAlert("Phone Number 2 must be exactly 10 digits.", "error");
    if (!/^\d+$/.test(whatsapp)) return showAlert("WhatsApp Number must contain only digits.", "error");
